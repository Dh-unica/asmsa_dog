...
$SSH_CMD "$REMOTE_USER@$REMOTE_HOST" "mkdir -p $REMOTE_TEMP_DIR && cd $REMOTE_WEBROOT && docker exec asmsa_dog_php bash -c 'mkdir -p /var/www/html/web/dump-tmp && drush sql-dump --gzip --result-file=/var/www/html/web/dump-tmp/$DUMP_NAME'"
